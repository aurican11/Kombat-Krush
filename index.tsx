import React, { useEffect, useCallback, useRef, useReducer, memo } from 'react';
...
const PieceIcons: { [key: string]: () => JSX.Element } = {
    scorpion: () => React.createElement("svg", { viewBox: "0 0 100 100" },
        React.createElement("g", { fill: "#FFFFFF", stroke: "#1a1a1a", strokeWidth: "4", strokeLinejoin: "round", strokeLinecap: "round" },
            React.createElement("path", { d: "M50,15 C 25,15 20,40 25,60 L 25,80 C 25,90 40,95 50,95 C 60,95 75,90 75,80 L 75,60 C 80,40 75,15 50,15 Z" }),
            React.createElement("circle", { cx: "40", cy: "50", r: "10", fill: "#1a1a1a", stroke: "none" }),
            React.createElement("circle", { cx: "60", cy: "50", r: "10", fill: "#1a1a1a", stroke: "none" }),
            React.createElement("path", { d: "M47,62 L53,62 L50,72 Z", fill: "#1a1a1a", stroke: "none" }),
            React.createElement("path", { d: "M35,80 H 65 M40,80 V 88 M45,80 V 88 M50,80 V 88 M55,80 V 88 M60,80 V 88", stroke: "#1a1a1a", strokeWidth: "2", fill: "none", strokeLinecap: "butt" })
        )
    ),
    subzero: () => React.createElement("svg", { viewBox: "0 0 100 100" },
        React.createElement("defs", null,
            React.createElement("g", { id: "snowflake-arm" },
                React.createElement("path", { d: "M50 42 L50 15" }),
                React.createElement("path", { d: "M50 35 L42 30" }),
                React.createElement("path", { d: "M50 35 L58 30" }),
                React.createElement("path", { d: "M50 22 L45 18" }),
                React.createElement("path", { d: "M50 22 L55 18" })
            )
        ),
        React.createElement("g", { stroke: "var(--subzero-color)", strokeWidth: "5", strokeLinecap: "round", strokeLinejoin: "round", fill: "none" },
            React.createElement("path", { d: "M50 42 L57 46 L57 54 L50 58 L43 54 L43 46 Z" }),
            React.createElement("use", { href: "#snowflake-arm" }),
            React.createElement("use", { href: "#snowflake-arm", transform: "rotate(60 50 50)" }),
            React.createElement("use", { href: "#snowflake-arm", transform: "rotate(120 50 50)" }),
            React.createElement("use", { href: "#snowflake-arm", transform: "rotate(180 50 50)" }),
            React.createElement("use", { href: "#snowflake-arm", transform: "rotate(240 50 50)" }),
            React.createElement("use", { href: "#snowflake-arm", transform: "rotate(300 50 50)" })
        )
    ),
    reptile: () => React.createElement("svg", { viewBox: "0 0 100 100" }, React.createElement("g", null, React.createElement("circle", { cx: "50", cy: "50", r: "40", fill: "var(--reptile-color)", stroke: "#1a1a1a", strokeWidth: "5" }), React.createElement("path", { d: "M50 20 C 60 35, 60 65, 50 80 C 40 65, 40 35, 50 20 Z", fill: "#1a1a1a" }), React.createElement("circle", { cx: "50", cy: "50", r: "30", fill: "none", stroke: "rgba(0,0,0,0.3)", strokeWidth: "6" }))),
    kano: () => React.createElement("svg", { viewBox: "0 0 100 100" }, React.createElement("g", { fill: "var(--kano-color)", stroke: "#1a1a1a", strokeWidth: "5" }, React.createElement("circle", { cx: "50", cy: "50", r: "40" }), React.createElement("circle", { cx: "50", cy: "50", r: "25", fill: "#fff" }), React.createElement("circle", { cx: "50", cy: "50", r: "10" }), React.createElement("path", { d: "M50 10 V 90 M10 50 H 90", stroke: "#1a1a1a", strokeWidth: "4", fill: "none", opacity: "0.6", strokeDasharray: "5,5" }))),
    raiden: () => React.createElement("svg", { viewBox: "0 0 100 100" },
        React.createElement("g", { fill: "#f8c838", stroke: "#1a1a1a", strokeWidth: "5", strokeLinejoin: "round", strokeLinecap: "round" },
            React.createElement("path", { d: "M50 10 L30 50 L45 50 L40 90 L70 40 L55 40 L50 10 Z" })
        )
    ),
    liukang: () => React.createElement("svg", { viewBox: "0 0 100 100" },
        React.createElement("g", { stroke: "#1a1a1a", strokeWidth: "5", strokeLinejoin: "round" },
            React.createElement("path", { fill: "var(--liukang-color)", d: "M50,95 C35,80 20,75 30,45 C35,25 45,15 50,10 C55,15 65,25 70,45 C80,75 65,80 50,95 Z" }),
            React.createElement("path", { fill: "#FFEB3B", d: "M50,85 C42,75 35,70 40,50 C42,38 48,30 50,25 C52,30 58,38 60,50 C65,70 58,75 50,85 Z" })
        )
    ),
};

const CHARACTER_DATA: { [key: string]: { name: string; description: string; ability: string; } } = {
    scorpion: {
        name: 'Scorpion',
...
const LOCAL_BANTER: { [key: string]: { [key: string]: string[] } } = {
    scorpion: {
...
const playSound = (type: string, options?: any) => {
    if (!audioContext) {
        try {
...
const createPiece = (row: number, col: number, type: string | null, idCounter: React.MutableRefObject<number>, special = 'none'): Piece => {
    return {
...
const findMatches = (board: Piece[][]) => {
    const matches: { pieces: Piece[]; type: string; length: number }[] = [];
    // Check rows
...
const hasPossibleMoves = (board: Piece[][]) => {
...
const applyGravity = (board: Piece[][], idCounter: React.MutableRefObject<number>): Piece[][] => {
...
const refillBoard = (board: Piece[][], idCounter: React.MutableRefObject<number>): Piece[][] => {
    const boardCopy = board.map(row => [...row]);
    for (let r = 0; r < GRID_SIZE; r++) {
        for (let c = 0; c < GRID_SIZE; c++) {
            if (boardCopy[r][c].type === null) {
...
const handleMatchClearingAndSpecials = (
    boardCopy: Piece[][],
    matches: { pieces: Piece[], type: string, length: number }[],
    swapLocation: { row: number, col: number } | null,
    dispatch: React.Dispatch<any>
) => {
    // FIX: Typed the Sets to Set<Piece> to resolve type errors in downstream operations like .reduce().
    ...
    let specialToCreate: { pieceToTransform: Piece, newSpecial: string } | null = null;
    const findSpecial = (match: { length: number, type: string }, p: Piece) => {
        if (match.length >= 5) return { pieceToTransform: p, newSpecial: 'dragon' };
        if (match.length === 4) return { pieceToTransform: p, newSpecial: match.type === 'row' ? 'row' : 'col' };
        return null;
    }
...
const processGameLoop = async ({ state, dispatch, playSoundMuted, pieceIdCounter, popupIdCounter, initialAction }: { state: State, dispatch: React.Dispatch<Action>, playSoundMuted: (...args: any[]) => void, pieceIdCounter: React.MutableRefObject<number>, popupIdCounter: React.MutableRefObject<number>, initialAction: any }) => {
    ...
    while (changedInLoop) {
        ...
        if (isInitialAbility && initialAction.type === 'ability') {
            ...
            if (selectedCharacter === 'liukang') {
                 ...
                 const otherPieceTypes = [...new Set(boardCopy.flat().map(p => p.type).filter(t => t && t !== playerPieceType))];
        ...
        if (matches.length > 0) {
            ...
            // FIX: Typed the `piece` parameter in the reduce function to resolve type error for `piece.type` and ensure `baseDamage` is a number.
            const baseDamage = [...allPiecesToClear].reduce((acc, piece: Piece) => acc + (piece.type === state.selectedCharacter ? 1.5 : 1), 0);
            ...
            // FIX: Explicitly provided a placeholder ID which will be overwritten in the reducer. This satisfies the type checker.
            ...
            let animationBoard = boardCopy.map(row => row.map(p => allPiecesToClear.has(p) ? {...p, state: 'matched'} : p));
            dispatch({ type: 'SET_BOARD', payload: animationBoard });
            await sleep(ANIMATION_DELAY * 3);

            let boardWithoutMatched = boardCopy.map(row => row.map((p) => {
...
// FIX: Added a comprehensive State interface to type the game state object.
interface State {
    board: Piece[][];
    gameState: 'start' | 'characterSelect' | 'playing' | 'levelWin' | 'ladderComplete' | 'gameOver';
    playerHealth: number;
...
// FIX: Added a comprehensive Action union type for all reducer actions.
type Action = 
  | { type: 'SET_GAME_STATE', payload: State['gameState'] }
  | { type: 'RESET_STATE' }
  | { type: 'START_GAME', payload: { character: string, ladder: Opponent[], board: Piece[][] } }
  ...
  | { type: 'ADD_TEXT_POPUP', payload: { popup: Omit<TextPopup, 'id'>, counterRef: React.MutableRefObject<number> } }
  ...
  | { type: 'PLAYER_BANTER', payload: { event: string, character: string, cooldownRef: React.MutableRefObject<any> } }
  | { type: 'OPPONENT_BANTER', payload: { event: string } }
  | { type: 'TOGGLE_MUTE' }
  | { type: 'TOGGLE_SETTINGS_MODAL' }
  | { type: 'CLEAR_HINTS' }
  | { type: 'SET_AUTO_HINT', payload: number[] }
  ...
  | { type: 'SET_ABILITY_STATE', payload: string }
...
function gameReducer(state: State, action: Action): State {
    switch (action.type) {
        ...
        case 'ADD_TEXT_POPUP':
            const newPopup = { ...action.payload.popup, id: action.payload.counterRef.current++ };
            return { ...state, textPopups: [...state.textPopups, newPopup] };
...
// FIX: Added prop types for the GamePiece component.
interface GamePieceProps {
    piece: Piece;
    onClick: () => void;
    isSelected: boolean;
    isHinted: boolean;
    isManualHinted: boolean;
    isCursorOn: boolean;
...
const GamePiece = memo(({ piece, onClick, isSelected, isHinted, isManualHinted, isCursorOn, onSwipe, isAiming, isAimTarget }: GamePieceProps) => {
...
// FIX: Added prop types for the EndGameModal component.
...
const EndGameModal = memo(({ gameState, opponent, maxCombo, score, onPlayAgain, onMainMenu, onNextLevel }: EndGameModalProps) => {
    ...
    return React.createElement("div", { className: "screen-overlay" },
        React.createElement("div", { className: `modal-dialog ${gameState}` },
            gameState === 'ladderComplete' && React.createElement("h2", { className: "victory-text perfect-victory-text" }, "FLAWLESS VICTORY"),
            gameState === 'levelWin' && React.createElement("h2", { className: "victory-text" }, "YOU WIN"),
            gameState === 'gameOver' && React.createElement("h2", { className: "fatality-text" }, "GAME OVER"),
            gameState === 'ladderComplete' ? React.createElement(React.Fragment, null,
                React.createElement("p", null, "You have defeated all challengers and conquered the ladder!"),
                React.createElement("button", { onClick: onPlayAgain }, "Play Again")
            ) : gameState === 'levelWin' ? React.createElement(React.Fragment, null,
                React.createElement("p", null, `You have defeated ${opponent?.name}. Prepare for the next battle!`),
                React.createElement("button", { onClick: onNextLevel }, "Next Fight")
            ) : React.createElement(React.Fragment, null,
                React.createElement("div", { className: "final-score-container" },
                    React.createElement("p", null, "Final Score: ", React.createElement("span", null, score)),
                    React.createElement("p", null, "Max Combo: ", React.createElement("span", null, "x", maxCombo))
                ),
                React.createElement("p", null, `You were defeated by ${opponent?.name}.`),
                React.createElement("div", { className: "modal-button-group" },
                    React.createElement("button", { onClick: onPlayAgain }, "Play Again"),
                    React.createElement("button", { onClick: onMainMenu, style: { backgroundColor: '#222', borderColor: '#555' } }, "Main Menu")
                )
            )
        )
    );
...
// FIX: Added prop types for the SettingsModal component.
interface SettingsModalProps {
    isOpen: boolean;
    onClose: () => void;
    onMainMenu: () => void;
}
const SettingsModal = memo(({ isOpen, onClose, onMainMenu }: SettingsModalProps) => {
    if (!isOpen) {
        return null;
    }
    return React.createElement("div", { className: "screen-overlay" },
        React.createElement("div", { className: "modal-dialog" },
            ...
            React.createElement("div", { className: "settings-modal-buttons" },
                React.createElement("button", { onClick: onMainMenu }, "Return to Main Menu"),
                React.createElement("button", { onClick: onClose, style: { backgroundColor: '#222', borderColor: '#555' } }, "Resume Game")
            )
        )
    );
});

// FIX: Added prop types for the CharacterSelectScreen component.
interface CharacterSelectScreenProps {
    onStartGame: (character: string) => void;
}
const CharacterSelectScreen = memo(({ onStartGame }: CharacterSelectScreenProps) => React.createElement("div", { className: "screen-overlay" },
    React.createElement("div", { className: "modal-dialog character-select" },
        React.createElement("h2", null, "Choose Your Fighter"),
        React.createElement("p", { className: "scroll-hint" }, "Scroll or swipe to see all fighters"),
        React.createElement("div", { className: "fighters-container" },
            (Object.keys(CHARACTER_DATA)).map(charKey => {
                const char = CHARACTER_DATA[charKey];
                return React.createElement("button", { key: char.name, className: `fighter-card ${charKey}`, onClick: () => onStartGame(charKey) },
                    React.createElement("h3", null, char.name),
                    React.createElement("div", { className: `char-portrait ${charKey}` }),
                    React.createElement("p", null, char.description)
                )
            })
        )
    )
...
// FIX: Added prop types for the StartScreen component.
interface StartScreenProps {
    onStart: () => void;
}
const StartScreen = memo(({ onStart }: StartScreenProps) => React.createElement("div", { className: "screen-overlay" },
    React.createElement("div", { className: "modal-dialog" },
        React.createElement("h1", null, "Kombat Krush"),
        React.createElement("p", null, "A Match-3 Fighting Game."),
        React.createElement("p", null, "Match pieces to damage your opponent. Defeat 5 opponents to win the tournament!"),
        React.createElement("button", { onClick: onStart }, "Start Game")
    )
));

// FIX: Added prop types for the PlayerProfile component.
interface PlayerProfileProps {
    selectedCharacter: string | null;
    playerBanter: { key: number; text: string; } | null;
    playerIsHit: boolean;
}
const PlayerProfile = memo(({ selectedCharacter, playerBanter, playerIsHit }: PlayerProfileProps) => React.createElement("div", { className: "player-info-col" },
    React.createElement("div", { className: "character-header" },
        React.createElement("div", { key: playerBanter?.key || 'player-banter-static', className: "player-banter-bubble", "aria-live": "polite" }, playerBanter ? `"${playerBanter.text}"` : ''),
        React.createElement("div", { className: `player-portrait ${selectedCharacter || ''} ${playerIsHit ? 'taking-damage' : ''}` })
    )
));

// FIX: Added prop types for the OpponentProfile component.
...
const OpponentProfile = memo(({ opponent, opponentBanter, opponentIsHit, movesUntilAttack, shuffledLadder, currentLadderLevel }: OpponentProfileProps) => React.createElement("div", { className: "opponent-info-col" },
    opponent && React.createElement(React.Fragment, null,
        React.createElement("div", { className: "character-header" },
            React.createElement("div", { key: opponentBanter?.key || 'opponent-banter-static', className: "opponent-banter-bubble", "aria-live": "polite" }, opponentBanter ? `"${opponentBanter.text}"` : ''),
            React.createElement("div", { className: `opponent-portrait ${opponent.pieceType || ''} ${opponentIsHit ? 'taking-damage' : ''}` })
        ),
        React.createElement("div", { className: "opponent-attack-timer" },
            React.createElement("div", { className: "attack-timer-label" }, "ATTACK IN"),
            React.createElement("div", { className: "attack-timer-value" }, movesUntilAttack)
        ),
        React.createElement("div", { className: "ladder-container" },
            shuffledLadder.map((opp, index) => React.createElement("div", {
                key: opp.pieceType,
                className: `ladder-portrait ${opp.pieceType} ${index < currentLadderLevel ? 'defeated' : ''} ${index === currentLadderLevel ? 'current' : ''}`
            }))
        )
    )
));

// FIX: Added prop types for the GameHeader component.
...
}: GameHeaderProps) => React.createElement("header", { className: "game-header" },
    React.createElement("div", { className: "health-bars-container" },
        React.createElement("div", { className: "health-bar-wrapper" },
            React.createElement("div", { className: "health-bar-label" }, selectedCharacter),
            React.createElement("div", { className: "health-bar-outer" },
                React.createElement("div", { className: "health-bar-inner player", style: { width: `${(playerHealth / PLAYER_MAX_HEALTH) * 100}%` } })
            )
        ),
        React.createElement("div", { className: "health-bar-wrapper" },
            React.createElement("div", { className: "health-bar-label" }, opponentName),
            React.createElement("div", { className: "health-bar-outer" },
                React.createElement("div", { className: "health-bar-inner opponent", style: { width: `${(opponentHealth / opponentMaxHealth) * 100}%` } })
            )
        )
    ),
    React.createElement("div", { className: "controls-and-meters" },
        selectedCharacter && React.createElement("div", { className: `ability-container ${selectedCharacter}` },
            React.createElement("div", { className: "ability-tooltip", "aria-hidden": "true" },
                React.createElement("strong", null, CHARACTER_DATA[selectedCharacter].name, " Ability:"),
                React.createElement("br", null),
                CHARACTER_DATA[selectedCharacter].description
            ),
            React.createElement("div", { className: "ability-meter-outer" },
                React.createElement("div", { className: "ability-meter-inner", style: { width: `${(abilityMeter / ABILITY_METER_MAX) * 100}%` } })
            ),
            React.createElement("button", { className: `ability-button ${abilityState}`, onClick: onAbilityClick, disabled: abilityState !== 'ready' || isProcessing }, "Ability")
        ),
        React.createElement("div", { className: "header-buttons" },
            React.createElement("button", { className: "hint-button", onClick: onHintClick, disabled: isHintOnCooldown || isProcessing }, isHintOnCooldown ? `(${hintCooldown}s)` : 'Hint'),
            React.createElement("button", { className: "mute-button", onClick: onMuteClick, "aria-label": isMuted ? 'Unmute' : 'Mute' },
                isMuted ?
                    React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "currentColor" }, React.createElement("path", { d: "M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" })) :
                    React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "currentColor" }, React.createElement("path", { d: "M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z" }))
            ),
            React.createElement("button", { className: "settings-button", onClick: onSettingsClick, "aria-label": "Settings" },
                React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "currentColor" }, React.createElement("path", { d: "M19.43 12.98c.04-.32.07-.64.07-.98s-.03-.66-.07-.98l2.11-1.65c.19-.15.24-.42.12-.64l-2-3.46c-.12-.22-.39-.3-.61-.22l-2.49 1c-.52-.4-1.08-.73-1.69-.98l-.38-2.65C14.46 2.18 14.25 2 14 2h-4c-.25 0-.46.18-.49.42l-.38 2.65c-.61.25-1.17.59-1.69.98l-2.49-1c-.23-.09-.49 0-.61.22l-2 3.46c-.13.22-.07.49.12.64l2.11 1.65c-.04.32-.07.65-.07.98s.03.66.07.98l-2.11 1.65c-.19.15-.24.42-.12.64l2 3.46c.12.22.39.3.61.22l2.49-1c.52.4 1.08.73 1.69.98l.38 2.65c.03.24.24.42.49.42h4c.25 0 .46-.18.49.42l.38-2.65c.61-.25 1.17-.59 1.69.98l2.49 1c.23-.09.49 0-.61.22l2-3.46c.12-.22.07-.49-.12-.64l-2.11-1.65zM12 15.5c-1.93 0-3.5-1.57-3.5-3.5s1.57-3.5 3.5-3.5 3.5 1.57 3.5 3.5-1.57 3.5-3.5 3.5z" }))
            )
        )
    )
));

// FIX: Added prop types for the GameBoard component.
...
const GameBoard = memo(({ board, specialEffects, textPopups, showToasty, selectedPiece, autoHintIds, manualHintIds, keyboardCursor, abilityState, aimTarget, onPieceClick, onSwipe }: GameBoardProps) => React.createElement("div", { className: "game-board-container" },
    React.createElement("div", { className: "game-board" },
        specialEffects.map(effect => React.createElement("div", {
            key: effect.id,
            className: `special-effect ${effect.type === 'row' ? 'effect-row' :
            ...
            effect.type === 'ice_shatter' && Array.from({ length: 6 }).map((_, i) => React.createElement("i", { key: i }))
        ))
    ),
    board.flat().map((piece) => React.createElement(GamePiece, {
        key: piece.id,
        piece: piece,
        onClick: () => onPieceClick(piece.row, piece.col),
        onSwipe: (direction) => onSwipe(piece.row, piece.col, direction),
        isSelected: !!selectedPiece && selectedPiece.row === piece.row && selectedPiece.col === piece.col,
        ...
        isAimTarget: !!aimTarget && aimTarget.row === piece.row && aimTarget.col === piece.col
    })),
    textPopups.map(popup => React.createElement("div", {
        key: popup.id,
    ...
    showToasty && React.createElement("div", { className: "toasty-container" },
        React.createElement("div", { className: "toasty-guy-head" }, "TOASTY!")
    )
));

// FIX: Added prop types for the MobileHeader component.
...
const MobileHeader = memo(({ selectedCharacter, playerBanter, playerIsHit, opponent, opponentBanter, opponentIsHit }: MobileHeaderProps) => React.createElement("div", { className: "mobile-header" },
    React.createElement("div", { className: "mobile-profile-item" },
        React.createElement("div", { key: playerBanter?.key || 'player-banter-static', className: "player-banter-bubble", "aria-live": "polite" }, playerBanter ? `"${playerBanter.text}"` : ''),
        React.createElement("div", { className: `player-portrait ${selectedCharacter || ''} ${playerIsHit ? 'taking-damage' : ''}` })
    ),
    opponent && React.createElement("div", { className: "mobile-profile-item" },
        React.createElement("div", { key: opponentBanter?.key || 'opponent-banter-static', className: "opponent-banter-bubble", "aria-live": "polite" }, opponentBanter ? `"${opponentBanter.text}"` : ''),
        React.createElement("div", { className: `opponent-portrait ${opponent.pieceType || ''} ${opponentIsHit ? 'taking-damage' : ''}` })
    )
));

// FIX: Added prop types for the MobileFooter component.
...
const MobileFooter = memo(({ opponent, movesUntilAttack, shuffledLadder, currentLadderLevel }: MobileFooterProps) => React.createElement("div", { className: "mobile-footer" },
    opponent && React.createElement(React.Fragment, null,
        React.createElement("div", { className: "opponent-attack-timer" },
            React.createElement("div", { className: "attack-timer-label" }, "ATTACK IN"),
            React.createElement("div", { className: "attack-timer-value" }, movesUntilAttack)
...
const App = () => {
    ...
    const hintTimerId = useRef<number | null>(null);
    const banterCooldownTimer = useRef<any>(null);
    
    // FIX: Typed the rest parameter `...args` to be a tuple, resolving the spread operator error.
    const playSoundMuted = useCallback((...args: [string, object?]) => {
        if (!isMuted) {
            playSound(...args);
        }
    }, [isMuted]);

    const generatePlayerBanter = useCallback((event: string) => {
    ...
    useEffect(() => {
        ...
        if (gameState === 'playing' && !isProcessing && !selectedPiece) {
            hintTimerId.current = window.setTimeout(() => {
    ...
    const startGame = useCallback((character: string) => {
    ...
    }, [state, playSoundMuted, gameState, isProcessing, selectedPiece, abilityState, dispatch]);
    ...
    }, [isProcessing, gameState, selectedPiece, handlePieceClick, abilityState, dispatch]);
    ...
    }, [state, playSoundMuted, generatePlayerBanter, abilityState, isProcessing, selectedCharacter, dispatch]);
    ...
    }, [gameState, isProcessing, keyboardCursor, handlePieceClick, handleAbilityClick, dispatch]);
    ...
    if (gameState === 'characterSelect') {
        return React.createElement(CharacterSelectScreen, { onStartGame: startGame });
    }

    return React.createElement(React.Fragment, null,
        React.createElement(EndGameModal, {
            gameState: gameState,
            opponent: opponent,
            maxCombo: maxCombo,
            score: score,
        ...
        React.createElement("div", { className: "game-ui-wrapper", "aria-hidden": !['playing'].includes(gameState) },
            ...
            React.createElement("div", { className: "game-container" },
                React.createElement("h1", { className: "main-title" }, "Kombat Krush"),
                ...
                combo >= 4 && React.createElement("div", { key: comboKey, className: "combo-display" }, "COMBO x", combo),
            ...
            React.createElement(OpponentProfile, {
                opponent: opponent,
                opponentBanter: opponentBanter,
                opponentIsHit: opponentIsHit,
            ...
            React.createElement(MobileFooter, {
                opponent: opponent,
                movesUntilAttack: movesUntilAttack,
                shuffledLadder: shuffledLadder,
                currentLadderLevel: currentLadderLevel
